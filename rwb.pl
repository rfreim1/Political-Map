...
if ($action eq "base") { 
  ...
  print "<div id=\"picker\" style=\"width:100\%; height:13\%\">",
	h3('Select Data'),
	"<form name=\"picker\">
	<input type =\"checkbox\" id = \"cand\" value = \"candidates\" onclick=\"ViewShift()\">Candidates</input><br />
	<input type =\"checkbox\" id = \"commit\" value = \"committees\" onclick=\"ViewShift()\" checked=\"checked\">Committees</input><br />
	<input type =\"checkbox\" id = \"indiv\" value = \"individuals\" onclick=\"ViewShift()\">Individuals</input><br />
	<input type =\"checkbox\" id = \"opin\" value = \"opinions\" onclick=\"ViewShift()\">Opinions</input><br />
	</form>
	
	<form class = \"right\">",
	h3('Select Cycle Years');

  #Get list of cycles then put their values in values to be sent to near action
  my @cycle_list;
  eval{
      @cycle_list = ExecSQL($dbuser, $dbpasswd, "select distinct cycle from cs339.individual group by cycle order by cycle", undef);
  };
  if (!$@){
    my $i;
    foreach my $cycle(@cycle_list){
      $i++;
      my $cy = @{$cycle}[0];
      my @splitcy = unpack("(A2)*", $cy);
      print "<input type =\"checkbox\" class = \"cycle\" value = \"$cy\" onclick=\"ViewShift()\">$splitcy[0]-$splitcy[1]</input>";
      #make space every 4 cycles
      if($i == 4) {
	print "<br />";
	$i = 0;
      }
    ...
    print "ERROR: cannot retrieve cycles";
  }
...
if ($action eq "near") {
  ...
  if ($len != 0){
    if ($what{committees}) { 
      my ($str,$error) = Committees($latne,$longne,$latsw,$longsw,@cycle_list,$format);
      if (!$error) {
	if ($format eq "table") { 
	  ...
	  my @money = TotalMoneyComm($latne,$longne,$latsw,$longsw,@cycle_list);
	  @money = $money[0];
	  $commMoney += $money[0];
	  while ($commMoney == 0){
	    $latne += .01;
	    ...
	    $latsw -= .01;
	    $longsw -= .01;
	    @money = TotalMoneyComm($latne,$longne,$latsw,$longsw,@cycle_list);
	    @money = $money[0];
	    $commMoney += $money[0];
	  }
	    my @dem_comm;
	    ...
	    my @rep_cand;
	    my $comm_color = "WHITE";
	    my $q = join(',', map( '?', @cycle_list));
	    eval{
	      @dem_comm = ExecSQL($dbuser, $dbpasswd, "select sum(transaction_amnt) from cs339.comm_to_comm natural join cs339.cmte_id_to_geo where cycle in ($q) and latitude>? and latitude<? and longitude>? and longitude<? and cmte_id in (select cmte_id from cs339.committee_master where cmte_pty_affiliation ='DEM')","COL",@cycle_list,$latsw,$latne,$longsw,$longne);
	    };
	    eval{
	      @rep_comm = ExecSQL($dbuser, $dbpasswd, "select sum(transaction_amnt) from cs339.comm_to_comm natural join cs339.cmte_id_to_geo where cycle in ($q) and latitude>? and latitude<? and longitude>? and longitude<? and cmte_id in (select cmte_id from cs339.committee_master where cmte_pty_affiliation ='REP')","COL",@cycle_list,$latsw,$latne,$longsw,$longne);
	    };
	    eval{
	      @dem_cand = ExecSQL($dbuser, $dbpasswd, "select sum(transaction_amnt) from cs339.comm_to_cand natural join cs339.cmte_id_to_geo where cycle in ($q) and latitude>? and latitude<? and longitude>? and longitude<? and cs339.comm_to_cand.cand_id in (select cand_id from cs339.candidate_master where cand_pty_affiliation ='DEM')","COL",@cycle_list,$latsw,$latne,$longsw,$longne);
	    };
	    eval{
	      @rep_cand = ExecSQL($dbuser, $dbpasswd, "select sum(transaction_amnt) from cs339.comm_to_cand natural join cs339.cmte_id_to_geo where cycle in ($q) and latitude>? and latitude<? and longitude>? and longitude<? and cs339.comm_to_cand.cand_id in (select cand_id from cs339.candidate_master where cand_pty_affiliation ='REP')","COL",@cycle_list,$latsw,$latne,$longsw,$longne);
	    };
	    if (!$@){
	      @dem_comm= $dem_comm[0];
	      @rep_comm = $rep_comm[0];
	      ...
	      if (($dem_comm[0] + $dem_cand[0]) > ($rep_comm[0] + $rep_cand[0])){
		$comm_color = "BLUE";
	      }
	      elsif (($dem_comm[0] + $dem_cand[0]) < ($rep_comm[0] + $rep_cand[0])){
		$comm_color = "RED";
	      }
	      else { $comm_color = "WHITE";
	      }
	  }
	  print "<h3>Committee Statistics</h3>";
	  print "<table border bgcolor=$comm_color>
	      <tr><td><b>Total</b></td></tr>
	      <tr><td>$commMoney</td></tr>
	      </table>";
      
    }
  }
    if ($what{candidates}) {
      my ($str,$error) = Candidates($latne,$longne,$latsw,$longsw,@cycle_list,$format);
      if (!$error) {
	if ($format eq "table") { 
    ...
    if ($what{individuals}) {
      my ($str,$error) = Individuals($latne,$longne,$latsw,$longsw,@cycle_list,$format);
      if (!$error) {
	if ($format eq "table") { 
	...
	my @money = TotalMoneyInd($latne,$longne,$latsw,$longsw,@cycle_list);
	    @money = $money[0];
	    $indMoney += $money[0];
	while ($indMoney == 0){
	  $latne += .01;
	  ...
	  $latsw -= .01;
	  $longsw -= .01;
	  @money = TotalMoneyInd($latne,$longne,$latsw,$longsw,@cycle_list);
	  @money = $money[0];
	  $indMoney += $money[0];
	}
	  my @dem_ind;
	  ...
	  my $q = join(',', map( '?', @cycle_list));
	  eval{
	    @dem_ind = ExecSQL($dbuser, $dbpasswd, "select sum(transaction_amnt) from cs339.individual natural join cs339.ind_to_geo where cycle in ($q) and latitude>? and latitude<? and longitude>? and longitude<? and cs339.individual.cmte_id in (select cmte_id from cs339.committee_master where cmte_pty_affiliation ='DEM')","COL",@cycle_list,$latsw,$latne,$longsw,$longne);
	  };
	  eval{
	    @rep_ind = ExecSQL($dbuser, $dbpasswd, "select sum(transaction_amnt) from cs339.individual natural join cs339.ind_to_geo where cycle in ($q) and latitude>? and latitude<? and longitude>? and longitude<? and cs339.individual.cmte_id in (select cmte_id from cs339.committee_master where cmte_pty_affiliation ='REP')","COL",@cycle_list,$latsw,$latne,$longsw,$longne);
	  };
	  if (!$@){
	    @dem_ind = $dem_ind[0];
	    @rep_ind = $rep_ind[0];
	    
	    if ($dem_ind[0] > $rep_ind[0]){
	      $ind_color = "BLUE";
	    }
	    elsif ($dem_ind[0] < $rep_ind[0]){
	      $ind_color = "RED";
	    }
	    else { $ind_color = "WHITE";
	    }
	    print "<h3>Individual Statistics</h3>";
	    print "<table border bgcolor=$ind_color>
	      <tr><th>Total</th></tr>
	      <tr><td>$indMoney</td></tr>
	      </table>";
	}
    ...
    if ($what{opinions}) {
      my ($str,$error) = Opinions($latne,$longne,$latsw,$longsw,$format);
      if (!$error) {
	if ($format eq "table") { 
	  ...
	  my $opinColor;
	  my @stat = AvgSTDOpinion($latne,$longne,$latsw,$longsw);
	    ...
	    while ($avg == 0){
	      ...
	      $latsw -= .01;
	      $longsw -= .01;
	      @stat = AvgSTDOpinion($latne,$longne,$latsw,$longsw);
	      $avg = $stat[0];
	      $avg = @{$avg}[0];
	    }
	    ...
	    if ($avg > 0){ $opinColor = "BLUE";}
	    elsif ($avg < 0){ $opinColor = "RED";}
	    else {$opinColor = "WHITE";}

	    print "<h3>Opinion Statistics</h3>";
	    print "<table border bgcolor=$opinColor>
  ...
  if (!UserCan($user,"invite-users")){
    ...
    if(!$run){
      print start_form(-name=>'InviteUser'),
	    ...
	    "Permissions: ", textfield(-name=>'permissions'),p
	    "(if more than 1 permission selected you MUST seperate them with COMMAS)",
	    hidden(-name=>'run',-default=>['1']),
	    ...
	    hr;
      my ($table,$error);
      ($table,$error)=SpecPermTable();
      if (!$error) { 
	print "<h2>Available Permissions to Give</h2>$table";
      }
    }
    else {
      my $address = param('email');
      my $perm = param('permissions');
      my $subject="RWB Account Invitation";
      my @chars = ("A".."Z", "a".."z", 0..9);
      my $rand = join("",@chars[map {rand @chars} (1..12)]);
      my $html = "<html><a href=\"rwb.pl?act=register&g=$rand\">Register</a></html>";
      my $content="Hello,\n\n$user has invited you to join the RWB community!\n\nClick the one-time link below to setup your account:\n\n
		   http://murphy.wot.eecs.northwestern.edu/~rhf687/rwb/rwb.pl?act=register&g=$rand&e=$address";
      my $error;
      my $randusername = join("",@chars[map {rand @chars} (1..20)]);
      $error=AddInvitee($randusername,"nonenone",$address,$user,$rand, $perm);
      if ($error) { 
	print "Can't add invitee because: $error <br /> Email not sent";
      }
      else{
	eval { open(MAIL,"| mail -s \'$subject\' $address") };
	if (@_){
	    print "Error: Can't send mail right now, try later.\n";
	}
	print MAIL $content;
	close(MAIL);
	print "Invite sent to $address referred by $user.\n";
      }
    }
  }
...
if ($action eq "register"){
  ...
  if (!ValidLink($email, $guid)){
	...
	print start_form(-name=>'Register'),
	    ...
	    "Password: ", password_field(-name=>'password'),p, "Password must be atleast 8 characters",
	    hidden(-name=>'run',-default=>['1']),
	    hidden(-name=>'act',-default=>['register']),p
	    submit,
	    end_form,
	    hr;
     }
	...
	my @perms = GetTempPerm($email);
	my $perm = $perms[0];
	if (length($password) < 8){
	   print "<b>ERROR</b>: Password is too short.<br />It must be ATLEAST 8 characters." 
	}
	else{
	  my $error;
	  $error = InviteeRegister($name, $password, $email);
	  if ($error) { 
	    print "Can't register invitee because: $error.";
	  }
	  else{
	    print "Registration sucessful!<br />";
	    my @perms2 = split(/,\s*/,$perm);
	    foreach my $perm(@perms2){
	     my $error = GiveUserPerm($name, $perm);
	     if ($error) { 
		print "Can't give permission because: $error.";
	      }
	    }
	  }
	}
...
if ($action eq "give-opinion-data") { 
  if (!UserCan($user,"give-opinion-data")){
    ...
    if(!$run){
      print start_form(-name=>'GiveOpinion', -id=>'opinion'),
	  h2("Give Opinion on Location"),
	  radio_group( 
		    -name=>'opinion',
		    -values=>[-1, 1, 0],
      ...
      my $error = AddOpinion($user, $opinion, $lat, $long);
      if ($error) { 
		print "Can't add opinion because: $error.";
      }
      else{
	print "Opinion data added! Thanks!";
      }
    }
  }
   print "<p><a href=\"rwb.pl?act=base&run=1\">Return</a></p>";
...
sub Committees {
  ...
  pop(@cycles);
  my $q = join(',', map( '?', @cycles));
  my @rows;
  eval { 
    @rows = ExecSQL($dbuser, $dbpasswd, "select latitude, longitude, cmte_nm, cycle, cmte_pty_affiliation, cmte_st1, cmte_st2, cmte_city, cmte_st, cmte_zip from cs339.committee_master natural join cs339.cmte_id_to_geo where cycle in ($q) and latitude>? and latitude<? and longitude>? and longitude<?",undef,@cycles,$latsw,$latne,$longsw,$longne);
  };
...
sub Individuals {
  ...
  my @rows;
  eval { 
    @rows = ExecSQL($dbuser, $dbpasswd, "select latitude, longitude, name, city, state, zip_code, employer, transaction_amnt from cs339.individual natural join cs339.ind_to_geo where cycle in ($q) and latitude>? and latitude<? and longitude>? and longitude<?",undef,@cycles,$latsw,$latne,$longsw,$longne);
  };
...
sub TotalMoneyComm {
  my ($latne,$longne,$latsw,$longsw,@cycles) = @_;
  my $q = join(',', map( '?', @cycles));
  my @rows;
  eval { 
    @rows = ExecSQL($dbuser, $dbpasswd, "select sum(transaction_amnt) as total from (select transaction_amnt from cs339.comm_to_cand where cs339.comm_to_cand.cmte_id in (select cmte_id from cs339.committee_master natural join cs339.cmte_id_to_geo where cycle in ($q) and latitude>? and latitude<? and longitude>? and longitude<?) union all select transaction_amnt from cs339.comm_to_comm where cs339.comm_to_comm.cmte_id in (select cmte_id from cs339.committee_master natural join cs339.cmte_id_to_geo where cycle in ($q) and latitude>? and latitude<? and longitude>? and longitude<?))","COL",@cycles,$latsw,$latne,$longsw,$longne,@cycles,$latsw,$latne,$longsw,$longne);
  };
  if ($@) { 
    return (undef,$@);
  } else {
...
sub TotalMoneyInd {
  ...
  my @rows;
  eval { 
    @rows = ExecSQL($dbuser, $dbpasswd, "select sum(transaction_amnt) as total from cs339.individual where cs339.individual.sub_id in (select sub_id from cs339.individual natural join cs339.ind_to_geo where cycle in ($q) and latitude>? and latitude<? and longitude>? and longitude<?)","COL",@cycles,$latsw,$latne,$longsw,$longne);
  };
  if ($@) { 
    return (undef,$@);
  } else {
...
#
# Generate a table of available permissions for a SPECIFIC user
# ($table,$error) = SpecPermTable()
# $error false on success, error string on failure
#
sub SpecPermTable {
  my @rows;
  eval { @rows = ExecSQL($dbuser, $dbpasswd, "select action from rwb_permissions where rwb_permissions.name=\'$user\'"); }; 
  if ($@) { 
    return (undef,$@);
  } else {
    return (MakeTable("spec_perm_table",
		      "2D",
		     ["Perm"],
		     @rows),$@);
  }
...
sub AddInvitee { 
  eval { ExecSQL($dbuser,$dbpasswd,
		 "insert into rwb_users (name,password,email,referer,guid,temp_perm) values (?,?,?,?,?,?)",undef,@_);};
  return $@;
}

sub InviteeRegister {
  eval { ExecSQL($dbuser,$dbpasswd,
		 "update rwb_users set name=?, password=?, guid=null where email=?",undef,@_);};
  return $@;
}
...
sub GetTempPerm {
  my @col;
  eval {
    @col= ExecSQL($dbuser,$dbpasswd, "select temp_perm from rwb_users where email=?","COL",@_);
  };
  if ($@) { 
    return undef;
  } else {
...
sub ValidLink {
  ...
  my @col;
  eval {
    @col= ExecSQL($dbuser,$dbpasswd, "select guid from rwb_users where email=?","COL",$email);
  };
  if ($@) { 
    return 0;
  } else {
    return $col[0] eq $guid;
  }
}
...
sub MakeColorTable {
  my ($id,$color,$headerlistref,@list)=@_;
  my $out;
  #
  ...
  if ((defined $headerlistref) || ($#list>=0)) {
    ...
    #
    $out="<table id=\"$id\" border bgcolor=\"$color\">";
    #
